...
function main() 
{
    ...
    mat4.translate(modelViewMatrix, modelViewMatrix, [-1.0, 0.65, -3.333]);
    ...
    mat4.identity(modelViewMatrix);
    mat4.translate(modelViewMatrix, modelViewMatrix, [1, 0.65, -3.333]);
    bindShaderAttributes(gl, shaderProgram);
...
function createPacman(gl)
{
    ...
    // hacemos un for para jugar con los angulos, tener la apertura de la boca del pacman
    for (let angle = 25; angle <= 315; angle++) {
        verts.push(Math.cos(angle * Math.PI / 180) * .5); // en x
        verts.push(Math.sin(angle * Math.PI / 180) * .5); // en y
        verts.push(0); // en z hacemos push de 0 para que sea 2D
    }
